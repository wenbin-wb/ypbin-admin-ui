...
class PreferenceManager {
  ...
  >({
    ...
    // accessMode 是项目固定配置（由 overrides 声明，如 backend/frontend），
    // 不属于用户可调偏好：强制采用 overrides 值，避免旧缓存里的 accessMode
    // 覆盖项目配置，导致菜单在 frontend(演示菜单) 与 backend(后端菜单) 间漂移。
    if (overrides?.app?.accessMode !== undefined) {
      mergedPreference.app = {
        ...mergedPreference.app,
        accessMode: overrides.app.accessMode,
      };
    }
